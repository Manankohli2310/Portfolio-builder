...
function toggleIconDialog(skill, parentElement, data, buildFormCallback) {
    ...
    dialog.innerHTML = `
        <div class="current-icon-preview">
            <div class="icon-display">${currentIconDisplay}</div>
        </div>
...
function buildTemplate1SkillsForm(formContainer, data, previewDoc, buildFormCallback) {
    if (data.skills.enabled) {
        ...
        masterToggleWrapper.innerHTML = `<label for="disable-all-icons-toggle">Enable Icons</label><input type="checkbox" id="disable-all-icons-toggle">`;
        const disableToggle = masterToggleWrapper.querySelector('#disable-all-icons-toggle');
        disableToggle.checked = data.skills.iconsEnabled;
        ...
        data.skills.list.forEach(skill => {
            ...
            input.oninput = (e) => {
                ...
                const updatedFinalIcon = data.skills.globalIconOverride || skill.iconClass;
                if (updatedFinalIcon && updatedFinalIcon.startsWith('data:image')) {
                    iconPreview.innerHTML = `<img src="${updatedFinalIcon}" alt="Custom Icon">`;
                } else {
                    iconPreview.innerHTML = `<i class="${updatedFinalIcon}"></i>`;
                }
                sendFullUpdate();
            };
