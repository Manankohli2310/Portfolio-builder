...
function buildTemplate1ExperienceForm(formContainer, data, previewDoc, buildFormCallback) {
    ...
    const section = createFormSection('Work Experience', 'experience', true);
    ...
    // --- THE KEY FIX IS HERE ---
    // Check if the user is below the limit of 10 experience items
    if (data.experience.list.length < 10) {
        const addBtn = document.createElement('button');
        addBtn.className = 'add-item-btn';
        addBtn.textContent = '+ Add Experience';
        addBtn.onclick = () => {
            // Add a new, completely blank experience object to the list
            data.experience.list.push({
                ...
                endYear: '',
                isPresent: false,
                points: [''] // Start with one empty point for the user
            });
            sendFullUpdate();
            buildFormCallback(); // Re-render the form with the new item
        };
...
function buildTemplate1ExperienceForm(formContainer, data, previewDoc, buildFormCallback) {
    ...
    // --- THE KEY FIX: Create the clickable header ---
    const header = document.createElement('div');
    header.className = 'form-section-header';
    header.innerHTML = `
        <h4>Work Experience</h4>
        <div class="header-controls" style="display: flex; align-items: center;">
            <button class="delete-section-btn" title="Remove Work Experience Section">&times;</button>
    ...
    // --- Populate data with defaults from the template ONCE if the list is empty ---
    if (data.experience.list.length === 0 && previewDoc.readyState === 'complete') {
        data.experience.list = Array.from(previewDoc.querySelectorAll('.timeline .timeline-item')).map((item, index) => {
            const companyElement = item.querySelector('.company');
            const dateText = companyElement ? companyElement.textContent.split('|')[1]?.trim() || '' : '';
